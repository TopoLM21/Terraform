...
public:
    struct Settings {
        ...
        // Доля от поверхностного давления (0..1), при которой прекращается генерация слоёв.
        // Физически означает, что выше порога атмосфера уже слишком разрежена, чтобы влиять
        // на массу/оптику и динамику, поэтому слои там не нужны.
        double minTopPressureFraction = 0.0;
