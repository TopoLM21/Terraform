...
constexpr double kSmoothingFactor = 0.35;
...
QVector<double> SurfaceAdvectionModel::advectTemperature(const PlanetSurfaceGrid &grid,
    ...
    for (int i = 0; i < pointCount; ++i) {
        ...
        // Температура в новой точке берётся в исходной точке траектории.
        ...
        advected[i] = temperatureK.at(bestIndex);
    }
    ...
    if (iterations > 0 && !neighborIndices_.isEmpty()) {
        ...
        for (int iter = 0; iter < iterations; ++iter) {
            for (int i = 0; i < pointCount; ++i) {
                ...
                // Стабилизирующее сглаживание (upwind/Laplacian blend):
                // T_{n+1} = T_adv + ν (T̄ - T_adv), где T̄ — среднее по соседям.
    ...
    const double floorTemperature = qMax(0.0, minTemperatureK);
    for (double &value : advected) {
        value = qMax(value, floorTemperature);
    }

    return advected;
}

void SurfaceAdvectionModel::ensureNeighbors(const PlanetSurfaceGrid &grid) const {
